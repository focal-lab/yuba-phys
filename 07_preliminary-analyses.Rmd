...
# predawn
```{r}
p2 <- ggplot(data = trait_data2, aes(x = DBH_complete, y = P50_MPa, color = species, linetype = elevation)) +
  ...
  scale_linetype_manual(values = c("low" = "twodash", "high" = "solid")) + 
  #scale_y_reverse() + 
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank()
...
library(MuMin)
dredge_df <- na.omit(trait_data2[, c("midday_MPa", "elevation", "DBH_complete", "percent_cover", "plot")])
global <- lmer(midday_MPa ~ elevation * DBH_complete * percent_cover + (1|plot), data = dredge_df)
dredge(global)
...
### stats
mod5 <- lmer(HSM_predawn ~ size + microsite + size:microsite + (1|plot), data = PIPO)
summary(mod5)
...
ggplot(trait_data1, aes(x = elev_indiv, y = P50_MPa, color = species)) +
  geom_point(size = 3, alpha = 1) +  
...
```{r}
mod5 <- lmer(P50_MPa ~size_class + elevation + (1|plot), 
             data = yuba_phys_P50_2024)
summary(mod5)
```
