...
s5 <- ggplot(trait_data2, aes(x = diameter_complete, y = HSM_midday_P50_mean, color = species)) +
  ...
  labs(
    x = "Diameter at root collar (cm)", y = "HSM midday (MPa)", color = "Species") +
    theme_bw() +
  scale_color_manual(values = c(
    "ABCO" = "mediumturquoise",
    "PIPO" = "lawngreen",
...
s5
grid.arrange(s1, s2, s3, s5, nrow = 2, ncol = 2)
...
mod <- lmer(P50_combined ~ diameter_complete + percent_cover + elev_indiv + (1 | plot), data = PIPO)
summary(mod)
